    ...
    fun (next: HttpFunc) (ctx: HttpContext) ->
        task {
            try
                ...
                let allErrors = 
                    (validationResults |> List.choose (function | Error e -> Some e | Ok _ -> None)) @
                    (optionalValidationResults |> List.choose (function | Error e -> Some e | Ok _ -> None))
                ...
                | [] ->
                    ...
                    if isSuspiciousContent report.ReportDescription report.ReportReason then
                        logger.LogWarning("Suspicious abuse report blocked, IP: {ClientIP}", ctx.Connection.RemoteIpAddress)
                        return! (setStatusCode 400 >=> json {| success = false; error = "Report contains suspicious content" |}) next ctx
                    else
                        ...
                        logger.LogWarning("Abuse report received - ID: {ReportId}, URL: {ReportedUrl}, Reason: {Reason}, Urgency: {Urgency}", reportId, report.ReportUrl, report.ReportReason, report.ReportUrgency)
                        
                        return! json {| success = true; reportId = reportId |} next ctx
                
                | errors ->
                    let errorMsg = String.concat "; " errors
                    logger.LogWarning("Invalid abuse report submission: {Error}, IP: {ClientIP}", errorMsg, ctx.Connection.RemoteIpAddress)
            ...
            | ex ->
                ...
                return! (setStatusCode 500 >=> json {| success = false; error = "Failed to submit report" |}) next ctx
        } 